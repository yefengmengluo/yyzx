	...
	<div class="t1000-votB-R-in clearfix">
    		...
    		<div class="t1000-siteManagement-tit clearfix">
                <h2 class="t1000-tit">留言管理</h2>
                 <form action="${pageContext.request.contextPath }/liuYanController/queryLiuYanLikeByKey?uPower=${sessionScope.user.userPower }&userId=${u.userId }" method="post" id="useradd" accept-charset="utf-8" class="vote-liuyan" >
	                <span>输入公司名称</span>
        ...
        <table class="memberList t1000-siteManagement" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td width="*"><a class="setData" href="${pageContext.request.contextPath }/liuYanController/queryOneByLYId?uPower=${sessionScope.user.userPower }&lyId=${u.lyId}&currentNum=${page.currentNum}&status=${status1}">编辑</a>
...
<script type="text/javascript">
	...
	var str="";
	 $("#batchDelete").click(function(){
		...
		if(confirm("确认删除吗")){
			...
			$.post(
			 		"${pageContext.request.contextPath }/liuYanController/removeLiuYan?lystatus="+lystatus,
					...
					//date:$("input[name='ids']:checked").serialize(),//$("input[type='checkbox'][id='checkBox']:checked").serialize(),
			 		function(){
			 			if(lystatus==""||lystatus==null){
						 	window.location.href = "${pageContext.request.contextPath }/liuYanController/queryAllLiuYanByPage?currentNum="+currentNum;	
							}else{
							window.location.href = "${pageContext.request.contextPath }/liuYanController/queryAllLiuYanByPageStatus?status="+lystatus+"&&currentNum="+currentNum;	
							}
		 			} 
					 ); 
		}else{
	 ...
	 $("#batchUpdate").click(function(){
		...
		if(confirm("确认更新吗？")){
			...
			$.post(
			 		"${pageContext.request.contextPath }/liuYanController/modifyLiuYanById?lystatus=1",
					{"lyIds":str,"uPower":uPower,"currentNum":on},
			 		function(){
						if(status1==""||status1==null){
					 	window.location.href = "${pageContext.request.contextPath }/liuYanController/queryAllLiuYanByPage?currentNum="+currentNum;	
						}else{
						window.location.href = "${pageContext.request.contextPath }/liuYanController/queryAllLiuYanByPageStatus?status="+status1+"&&currentNum="+currentNum;	
						}
		 			} 
					 ); 
		}else{
	...
	function pageSkip(pageNum){
			location.href="${pageContext.request.contextPath }/liuYanController/queryAllLiuYanByPage?currentNum="+pageNum;
		}
	
	function download(){
	    var url="${pageContext.request.contextPath }/liuYanController/download_project";
	    window.open(url);
	}
